...
class LinkedList:
    ...
    def partition_list(self, x):
        ...
        while pivot.value < x and pivot.next is not None:
            pre = pivot
            pivot = pivot.next

        to_shift = pivot
        while to_shift is not None and to_shift.next is not None:
            to_shift_pre = to_shift
            to_shift = to_shift.next

            if to_shift.value < x:
                if pre is None:
                    temp = to_shift.next
                    to_shift.next = self.head
                    self.head = to_shift
                    to_shift_pre.next = temp
                    pre = self.head
                else:
                    pre.next = to_shift
                    temp = to_shift.next
...
def test_partition_list():
    ...
    # Test 8: Empty List
    print("Test 8: Empty List, Equal to Partition")
    x = 3
    print(f"x = {x}")
    ll = LinkedList(3)
    ll.make_empty()
